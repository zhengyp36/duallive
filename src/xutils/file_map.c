...
do_map(file_map_t *fm)
{
	...
	size_t size = fi.size;
	if (!size || size > (1ULL << FM_SIZE_BITS) - 1) {
		printf("Error: *** error size of %s %ld.\n", fm->path, size);
		return (-1);
	}

	int fd = open(fm->path, fm->mode != FM_NORMAL ? O_RDONLY : O_RDWR);
	if (fd < 0) {
	...
	fm->buf = mmap(NULL, calc_map_size(size),
	    PROT_READ | (fm->mode == FM_RDONLY ? 0 : PROT_WRITE),
	    (fm->mode != FM_PRIVATE ? MAP_SHARED : MAP_PRIVATE), fd, 0);
	close(fd);
...
int
file_map_open_(file_map_t *fm, int quiet, int reopen, int mode, ...)
{
	if (fm_mapped(fm) && !reopen && fm->mode == mode)
		return (0);

	if (access(fm->path, F_OK)) {
		if (mode != FM_NORMAL || reopen) {
			fmlog(quiet, "Error: ** '%s' does not exit.\n",
			    fm->path);
			return (-1);
		}
...
int
file_map_sync(file_map_t *fm)
{
	int fd = open(fm->path, O_RDWR);
	if (fd < 0) {
		printf("Error: *** open %s error %d.\n", fm->path, errno);
		return (-1);
	}
